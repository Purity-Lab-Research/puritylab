        ...
        >
          {filtered.map((protocol) => {
            ...
            return (
              ...
              >
                {/* Colored header band */}
                <div className={`${colors.bg} px-5 pt-5 pb-4 relative`}>
                  {protocol.badge && (
                    <span
                      className={`absolute top-3 right-3 text-[10px] font-bold tracking-wider px-2.5 py-1 rounded-full ${BADGE_STYLES[protocol.badge] ?? "bg-[#10B981] text-white"}`}
                    >
                      {protocol.badge}
                    </span>
                  )}
                  <h3 className="text-lg font-bold text-[#111111] pr-20 leading-tight">
                    {protocol.name}
                  </h3>
